...
- **`deploy/caddy/Caddyfile`** — shared Caddy base with a top-level `/etc/caddy/sites.d/*.caddy` import, TLS, `/api/*` proxy, rate limits, strict security headers and privacy-preserving logs. Other domains own separate files that the hashgram.io installer checksum-verifies and never changes; `deploy/caddy/sites/hashgram.org.caddy` is the static companion-site reference.
